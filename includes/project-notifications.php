...
function wp_crm_system_notify_email_projects( $ID, $post ){
	...
	if( $user != '' ){
		...
		$email		= __( 'Not assigned', 'wp-crm-system-email-notifications' );
		$name		= __( 'Not assigned', 'wp-crm-system-email-notifications' );
	}
	...
	if( isset( $statusargs[$status] ) ) {
		...
		$sendstatus = __( 'Not set', 'wp-crm-system-email-notifications' );
	}
